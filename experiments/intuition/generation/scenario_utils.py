...
"""
```
$ python -m generation.scenario_utils --help
...
$ python -m generation.scenario_utils scenarios --help
...
$ python -m generation.scenario_utils validate --help
usage: scenario_utils.py validate [-h] --filename FILENAME [--qualification]

options:
  -h, --help           show this help message and exit
  --filename FILENAME  The HIT csv as generated by `python -m generation.scenario_utils scenarios`
  --qualification      Whether or not this file is from a qualification task.
$ python -m generation.scenario_utils release --help
...
$ python -m generation.scenario_utils cancel --help
...
options:
  ...
  --sandbox            Whether to use the sandbox environment.
  --self-assign        Assigns all of the HITs to the Alexandria account. Defaults to True if --sandbox.
$ python -m generation.scenario_utils results --help
...
options:
  ...
  --sandbox            Whether to use the sandbox environment.
  --self-assign        Assigns all of the HITs to the Alexandria account. Defaults to True if --sandbox.
$ python -m generation.scenario_utils workers --help
...
import pandas as pd
import tqdm
from common.shared_analysis import control, mturk_explode_df, test
from common.utils import (
    PROBABILITY_COLUMNS,
    QUESTION_STRINGS,
    explode_paginated_call,
    filename_to_variables,
)

import value_aggregation as pm

AGENTS_TEMPLATES = ["A", "B", "C"]
...
QUAL_TO_IDS = {
    "moral-parliament-q": "3XLL3006OQKVHYD6V2X7384QGQD97R",
...
SANDBOX_QUAL_IDS = {
    "moral-parliament-q": "31O6ZS77USI6TUQEOD56C22AGKE7WI",
...
def write_to_csv(hit_list, filename, num_scenarios):
    """
    Write a scenario list to CSV for MTurk, with one row per HIT.
    """

    scenarios = list(range(1, num_scenarios + 1))

    # First, discover all possible field names from the data
    all_keys = set()
    for hits in hit_list:
        for hit in hits:
            all_keys.update(hit.keys())

    # Now build the full list of CSV fieldnames
    fieldnames = []
    for i in scenarios:
        for key in sorted(list(all_keys)):
            fieldnames.append(f"scenario_{i}_{key}")
...
def _collect_joint_values(fehr_value, nbs_value, util_value, use_nash, use_util):
    """
    Given the per-game recommendations for Fehr, Nash, and Util (some optional),
    return a list of all values that should be considered jointly when defining
    agreements/disagreements, or ``None`` if any required value is undefined.
    """
    if fehr_value is None:
        return None
    values = [fehr_value]
    if use_nash:
        if nbs_value is None:
            return None
        values.append(nbs_value)
    if use_util:
        if util_value is None:
            return None
        values.append(util_value)
    return values


def _is_joint_agreement(values):
    """
    Return True if all provided recommendations agree (are identical).
    """
    return len(set(values)) == 1


def _is_joint_disagreement(values, use_nash, use_util):
    """
    Return True if the provided recommendations count as a disagreement under
    the current aggregation setup.

    - When using Fehr, Nash, and Util together (``use_nash`` and ``use_util``
      are both True), treat a disagreement as a *triple disagreement*, i.e.
      all three recommendations differ.
    - Otherwise (e.g., Fehr + Nash only), treat any non-unanimous set of
      recommendations as a disagreement.
    """
    unique_values = set(values)
    if use_nash and use_util and len(values) == 3:
        return len(unique_values) == 3
    return len(unique_values) > 1


def _log_scenario_file_summary(hit_list, num_scenarios, total_unique=None):
    """
    Log a brief summary of the generated scenario file:
    - number of rows (HITs),
    - number of unique scenarios,
    - how many scenarios per row, and
    - how many times each unique scenario appears in the file.
    """
    num_rows = len(hit_list)

    actual_total_unique = total_unique
    if actual_total_unique is None:
        unique_hashes = set()
        for hits_in_row in hit_list:
            for scenario in hits_in_row:
                unique_hashes.add(scenario["hash"])
        actual_total_unique = len(unique_hashes)

    if actual_total_unique > 0:
        total_scenario_slots = num_rows * num_scenarios
        appearances_per_unique = total_scenario_slots / actual_total_unique
    else:
        appearances_per_unique = 0

    logging.info(
        "Scenario file rows (HITs): %d; unique scenarios: %d; "
        "scenarios per row: %d; appearances per unique scenario: %.2f.",
        num_rows,
        actual_total_unique,
        num_scenarios,
        appearances_per_unique,
    )


def _resolve_disagreements_per_hit(
    disagrees_only, num_scenarios, disagreements_per_hit
):
    """Return the disagreement count per HIT row under current settings."""
    if disagrees_only:
        return num_scenarios

    if disagreements_per_hit is None:
        return math.floor(num_scenarios / 2)

    if disagreements_per_hit < 0 or disagreements_per_hit > num_scenarios:
        raise ValueError("disagreements_per_hit must be in [0, number_scenarios].")
    return disagreements_per_hit


def _log_multi_fehr_summary(
    alphas,
    totals_by_alpha,
    disagreements_by_alpha,
    nbs_cache,
    util_cache,
    fehr_cache,
    all_games,
    ...
    logging.info("Fehr alpha disagreement summary:")
    logging.info(
        "%5s %12s %12s %12s %14s %14s %14s %14s",
        "alpha",
        "total_all",
        "dis_all",
        "prop_all",
    ...
    use_nash = len(nbs_cache) > 0
    use_util = len(util_cache) > 0

    for alpha in alphas:
        total_all = totals_by_alpha[alpha]
        disagree_all = len(disagreements_by_alpha[alpha])
        prop_all = 0.0 if total_all == 0 else disagree_all / total_all

        total_selected = 0
        disagree_selected = 0
        for game in all_games:
            fehr_value = fehr_cache.get(game, {}).get(alpha)
            nbs_value = nbs_cache.get(game)
            util_value = util_cache.get(game)

            values = _collect_joint_values(
                fehr_value=fehr_value,
                nbs_value=nbs_value,
            ...
            total_selected += 1
            if _is_joint_disagreement(values, use_nash=use_nash, use_util=use_util):
                disagree_selected += 1
        prop_selected = (
            0.0 if total_selected == 0 else disagree_selected / total_selected
        )

        explicit_selected = len(selected_by_alpha[alpha])
        logging.info(
            "%5.1f %12d %12d %12.4f %14d %14d %14d %14.4f",
            alpha,
            total_all,
            disagree_all,
            prop_all,
    ...
    logging.info(
        "Column key: total_all/dis_all/prop_all are over all generated games; "
        "target_dis is the per-alpha disagreement target; "
        "exp_sel is how many unique games were explicitly selected for that alpha; "
        "dis_sel/prop_sel are disagreements among the final combined scenario set."
    )


def _make_multi_fehr_scenarios(agents, actions, args, action_function, num_scenarios):
    """
    Generate alpha-exclusive participant rows for Fehr-vs-Nash comparisons.

    For disagree trials, each row is assigned to exactly one alpha value and all
    disagree scenarios in that row are evaluated against IE(alpha) vs Nash.
    For agree trials, scenarios are restricted to cases where Nash and all
    Fehr alpha values agree on the same recommendation.
    """
    games = pm.generate_games(
        agents,
        actions,
    ...
    if args.prevent_ties:
        no_ties = set()
        for game in games:
            nbs_value = pm.run_nash_bargain(gameState=game, none_if_tie=True)
            if nbs_value is None:
                continue
            no_ties.add(game)
        games = no_ties
    ...
    use_nash = "nash" in args.aggregation_functions
    if not use_nash:
        raise ValueError(
            "Multi-fehr alpha-exclusive generation requires 'nash' in "
            "aggregation-functions."
        )

    disagreements_by_alpha = {alpha: [] for alpha in alphas}
    agreements_all_alpha = []
    nbs_cache = {}
    fehr_cache = {}
    ...
    for game in games:
        ...
        if nbs_value is None:
            continue
        nbs_cache[game] = nbs_value

        alpha_values = {}
        valid_for_all_alphas = True
        for alpha in alphas:
            fehr_value = pm.run_equality_efficiency(
                gameState=game,
                none_if_tie=True,
                alpha=alpha,
            )
            if fehr_value is None:
                valid_for_all_alphas = False
                break
            alpha_values[alpha] = fehr_value

        if not valid_for_all_alphas:
            continue

        fehr_cache[game] = alpha_values
        agree_all = True
        for alpha, fehr_value in alpha_values.items():
            if fehr_value != nbs_value:
                disagreements_by_alpha[alpha].append(game)
                agree_all = False
        if agree_all:
            agreements_all_alpha.append(game)

    disagreements_per_hit = _resolve_disagreements_per_hit(
        disagrees_only=args.disagrees_only,
        num_scenarios=num_scenarios,
        disagreements_per_hit=args.disagreements_per_hit,
    )
    agreements_per_hit = num_scenarios - disagreements_per_hit

    min_pool_size = min(len(disagreements_by_alpha[alpha]) for alpha in alphas)
    if disagreements_per_hit > 0 and min_pool_size < disagreements_per_hit:
        raise ValueError(
            "Not enough disagreements for at least one alpha to satisfy "
            f"disagreements_per_hit={disagreements_per_hit}. "
            f"Minimum available pool is {min_pool_size}."
        )

    rows_per_alpha_target = max(1, args.sample_size)
    rows_per_alpha = rows_per_alpha_target
    if disagreements_per_hit > 0:
        rows_per_alpha = min(rows_per_alpha_target, min_pool_size)
        if rows_per_alpha < rows_per_alpha_target:
            logging.warning(
                "Requested sample_size=%d rows per alpha but only %d are "
                "available for at least one alpha. Using %d rows per alpha.",
                rows_per_alpha_target,
                min_pool_size,
                rows_per_alpha,
            )

    if agreements_per_hit > 0 and len(agreements_all_alpha) < agreements_per_hit:
        raise ValueError(
            "Not enough all-alpha agreement scenarios to satisfy "
            f"agreements_per_hit={agreements_per_hit}. "
            f"Available all-alpha agreements: {len(agreements_all_alpha)}."
        )

    def encode_hit_for_fehr(game_state):
        """
        Encode a scenario with per-alpha Fehr recommendations.
        """
        sce_bytes = pm.encode_gameState(game_state)
        result = {
            "hash": hash(game_state),
            "json": sce_bytes,
            "mec": pm.run_mec(game_state, none_if_tie=True),
            "nbs": pm.run_nash_bargain(game_state, none_if_tie=True),
            "mft": pm.run_mft(game_state, none_if_tie=True),
        }
        for alpha_value in alphas:
            alpha_key = str(alpha_value).replace(".", "_")
            fehr_value = fehr_cache.get(game_state, {}).get(alpha_value)
            result[f"fehr_alpha_{alpha_key}"] = fehr_value if fehr_value else ""
        assert result["mec"] == pm.run_mec(game_state, none_if_tie=True)
        assert result["nbs"] == pm.run_nash_bargain(game_state, none_if_tie=True)
        return result

    encoded_cache = {
        game: encode_hit_for_fehr(game) for game in games if game in fehr_cache
    }

    def annotate_hit_for_alpha(hit, alpha_value):
        """
        Attach alpha-specific metadata for downstream analysis.
        """
        alpha_key = str(alpha_value).replace(".", "_")
        annotated = dict(hit)
        annotated["alpha_bin"] = alpha_value
        annotated["ie"] = hit.get(f"fehr_alpha_{alpha_key}", "")
        return annotated

    agreement_bank_size = 0
    selected_agreement_games = []
    if agreements_per_hit > 0:
        agreement_bank_size = min(
            len(agreements_all_alpha),
            max(rows_per_alpha, agreements_per_hit),
        )
        selected_agreement_games = random.sample(
            agreements_all_alpha,
            agreement_bank_size,
        )
    ...
    hit_list = []
    for alpha in alphas:
        disagreement_games = disagreements_by_alpha[alpha]
        if disagreements_per_hit > 0:
            selected_disagreement_games = random.sample(
                disagreement_games, rows_per_alpha
            )
            disagreement_hits = [
                annotate_hit_for_alpha(encoded_cache[game], alpha)
                for game in selected_disagreement_games
            ]
            disagreement_rows = pm.split_by(disagreement_hits, disagreements_per_hit)
        else:
            disagreement_rows = [[]]
        ...
        if agreements_per_hit > 0:
            agreement_hits = [
                annotate_hit_for_alpha(encoded_cache[game], alpha)
                for game in selected_agreement_games
            ]
            agreement_rows = pm.split_by(agreement_hits, agreements_per_hit)
        else:
            agreement_rows = [[]]

        if disagreements_per_hit > 0:
            alpha_row_count = len(disagreement_rows)
        else:
            alpha_row_count = len(agreement_rows)
        alpha_row_count = max(alpha_row_count, 1)

        for row_index in range(alpha_row_count):
            row = []
            row += list(disagreement_rows[row_index % len(disagreement_rows)])
            row += list(agreement_rows[row_index % len(agreement_rows)])
            random.shuffle(row)
            hit_list.append(row)

        logging.info(
            "alpha=%s rows=%d disagree_pool=%d agreement_bank=%d",
            alpha,
            alpha_row_count,
            len(disagreement_games),
            agreement_bank_size,
        )
    ...
    total_unique = len(
        set(tuple(sorted(hit.items())) for row in hit_list for hit in row)
    )

    _log_scenario_file_summary(
        hit_list=hit_list,
        num_scenarios=num_scenarios,
        total_unique=total_unique,
    )

    actual_sample = len(hit_list)
    return hit_list, actual_sample
...
def make_scenarios_cmd(args):
    ...
    aggregation_functions = [
        AGGREGATION_FUNCTIONS[name] for name in args.aggregation_functions
    ]
    ...
    use_multi_fehr = (
        not args.greedy_generation
        and "fehr" in args.aggregation_functions
        and (
            "nash" in args.aggregation_functions or "util" in args.aggregation_functions
        )
    )

    if use_multi_fehr:
        hit_list, actual_sample = _make_multi_fehr_scenarios(
            agents=agents,
            actions=actions,
            args=args,
        ...
        if args.greedy_generation:
            hit_list = pm.generate_hits_greedy(
                agents,
                actions,
                belief_range_steps=args.belief_steps,
                belief_range=args.belief_range,
        ...
        actual_sample = len(hit_list) if args.disagrees_only else len(hit_list) / 2
    ...
    csv_name += (
        f"_prevent-ties={args.prevent_ties}_agg-functions={args.aggregation_functions}"
    )
...
def query_qualified_workers(client):
    ...
    submitted = explode_paginated_call(
        client.list_workers_with_qualification_type,
        "Qualifications",
    ...
    has_question = explode_paginated_call(
        client.list_workers_with_qualification_type,
        "Qualifications",
    ...
    assigned_revoked = explode_paginated_call(
        client.list_workers_with_qualification_type,
        "Qualifications",
    ...
    if len(submitted_df) > 0:
        quals_not_submitted = quals_df[
            (~quals_df["WorkerId"].isin(submitted_df["WorkerId"]))
        ]
    else:
        quals_not_submitted = quals_df
    ...
    logging.info(
        f"{len(available_workers)} after allowing only those with a score >= 13."
    )
    ...
    if len(assigned_revoked) > 0:
        assigned_revoked_df = pd.DataFrame(assigned_revoked).rename(
            columns={"IntegerValue": "AssignedRevoked"}
        )
        workers = (
            workers.merge(assigned_revoked_df, on="WorkerId", how="left")
            .fillna(0)
        ...
        if 0 not in workers["AssignedRevoked"].value_counts():
            ...
            logging.info(
                f"{workers['AssignedRevoked'].value_counts()[0]} unassigned workers left!"
            )
...
def scenario_file_to_mturk(in_file, out_file, num_assignments, num_scenarios):
    ...
    for i in range(1, num_scenarios + 1):
        ...
        new_cols += [ans_q_col, f"{ans_q_col}_attn", f"{ans_q_col}_attn_answer"]
...
def mturk_html(args):
    ...
    if not os.path.exists(results_filename):
        scenario_file_to_mturk(
            args.filename, results_filename, args.assignments, num_scenarios
        )
    ...
    if args.self_assign:
        workers = {
            "WorkerId": [ALEXANDRIA_MTURK_WORKER_ID] * num_assignments,
    ...
    if len(potential_workers) < num_assignments:
        raise ValueError(
            (
                f"Pool of potential workers ({len(potential_workers)})"
    ...
    assignments = pd.concat(
        [selected_workers, remaining_assignments.reset_index(drop=True)], axis=1
    )
    ...
    try:
        for i, row in tqdm.tqdm(assignments.iterrows(), total=len(assignments)):
            ...
            actual_id = response["HIT"]["QualificationRequirements"][0][
                "IntegerValues"
            ][0]
            if actual_id != row["IntegerId"]:
                ...
                response = client.update_expiration_for_hit(
                    HITId=row["HITId"], ExpireAt=expiration
                )
...
def cancel_hits(args):
    ...
    results["Expiration"] = pd.to_datetime(
        results["Expiration"], format="mixed"
    ).dt.tz_localize(None)
    ...
    for i, row in tqdm.tqdm(results.iterrows(), total=len(results)):
        if (
            not pd.isnull(row["HITId"])
            and row["Expiration"] > datetime.datetime.now()
            and (
                pd.isnull(row["AssignmentStatus"]) or len(row["AssignmentStatus"]) == 0
            )
        ):
...
def check_results(args):
    ...
    released["Expiration"] = pd.to_datetime(
        released["Expiration"], format="mixed"
    ).dt.tz_localize(None)
    ...
    try:
        for i, row in tqdm.tqdm(released.iterrows(), total=len(released)):
            ...
            if (
                len(assignments) == 0
                or pd.isnull(assignments[0]["AssignmentStatus"])
                or len(assignments[0]["AssignmentStatus"]) == 0
            ):
                unfinished += 1
                if row["Expiration"] < datetime.datetime.now():
                    try:
                        ...
                        if (
                            not "This operation can be called with a status of: Granted"
                            in str(e)
                        ):
                            raise e
                    ...
                    try:
                        ...
                        response = client.get_qualification_score(
                            WorkerId=row["WorkerId"],
                            QualificationTypeId=QUAL_IDS[
                                "moral-parliament-q-assigned-revoked"
                            ],
                        )
                        ...
                        if (
                            not "You requested a Qualification that does not exist."
                            in str(e)
                        ):
                            raise e
            ...
            if assignment["WorkerId"] != row["WorkerId"]:
                logging.error(
                    f"The wrong worker, {assignment['WorkerId']} not {row['RequesterAnnotation']} submitted this HIT."
                )
                wrong_submitted += 1
            ...
            try:
                ...
                if "This operation can be called with a status of: Granted" not in str(
                    e
                ):
                    raise e
    finally:
        logging.info(
            f"{unfinished} assignments unfinished out of {len(released)} total."
        )
    ...
    if args.qualification:
        ...
        for _, row in tqdm.tqdm(df.iterrows(), total=len(df)):
            ...
            try:
                ...
                if not "This operation can be called with a status of: Granted" in str(
                    e
                ):
                    raise e
                else:
        ...
        df = df.merge(
            pd.concat([approved, rejected, submitted], axis=1),
            left_index=True,
            right_index=True,
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="A collection of utilities for running mturk experiments."
    )

    subparsers = parser.add_subparsers(dest="cmd", required=True)

    make_template_parser = subparsers.add_parser(
        "scenarios", help="Makes the scenario csv files."
    )
    make_template_parser.add_argument(
        "-m",
        "--minimize",
        default=False,
        action="store_true",
    ...
    make_template_parser.add_argument(
        ...
        required=True,
        help=(
            "Target number of scenario rows to generate. In multi-fehr mode "
            "this is interpreted as rows per alpha bin before availability caps."
        ),
    )
    make_template_parser.add_argument(
        "--prevent-ties",
        action="store_true",
        default=False,
        help="Whether to prevent ties.",
    )
    make_template_parser.add_argument(
        "--disagrees-only",
        action="store_true",
        default=False,
        help="Whether to only store disagreements.",
    )
    make_template_parser.add_argument(
        "--disagreements-per-hit",
        type=int,
        default=None,
        help=(
            "Fixed number of disagreement scenarios per participant row when "
            "not using --disagrees-only. Default is floor(number-scenarios/2)."
        ),
    )
    make_template_parser.add_argument(
        "--greedy-generation",
        action="store_true",
        default=False,
        help="Whether to greedily instatiate scenarios and not enumerate them all.",
    )
    make_template_parser.add_argument(
        "--aggregation-functions",
        nargs="+",
    ...
    make_template_parser.add_argument(
        ...
        default=None,
        help=(
            "Optional explicit Fehr alpha grid (for example: 0 0.25 0.5 0.75 1). "
            "Used in multi-fehr generation; defaults to 0.0..1.0 in 0.1 steps."
        ),
    )
    make_template_parser.set_defaults(func=make_scenarios_cmd)

    process_mturk_parser = subparsers.add_parser(
        "validate", help="Processes the HIT csv. Adds award and qual columns."
    )
    process_mturk_parser.add_argument(
        "--filename",
        required=True,
        help="The HIT csv as generated by `python -m generation.scenario_utils scenarios`",
    )
    process_mturk_parser.add_argument(
        "--qualification",
        default=False,
    ...
    make_mturk_html.add_argument(
        "--assignments", default=3, help="The number of workers to complete each HIT."
    )
    make_mturk_html.add_argument(
        "--sandbox",
        default=False,
    ...
    results = subparsers.add_parser(
        "results", help="Saves the results of the associated HIT if it exists."
    )
    ...
    workers = subparsers.add_parser(
        "workers", help="Reports stats on the worker population"
    )
    ...
    workers = subparsers.add_parser(
        "reset", help="Clears out all of the assigned worker qualification ids"
    )
    workers.add_argument(
        "--sandbox",
        default=False,
