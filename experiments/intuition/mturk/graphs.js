function make_legend(domain, color) {
  ...
  svg
    ...
    .attr("y", function (d, i) {
      return i * (lg_sq_size + 5) + lg_sq_size / 2;
    })
    // 100 is where the first dot appears. 25 is the distance between dots
    .style("fill", (d) => color(d))
    .text((d) => d)
...
function make_area_chart(data, color, width, height) {
  ...
  const groups = d3.groupSort(
    data,
    (D) => -d3.sum(D, (d) => d.utility),
    (d) => d.agent,
  );
  ...
  svg
    ...
    .attr(
      "width",
      (d) =>
        widthScale(xPos.get(d.data[0]).endAngle) -
        widthScale(xPos.get(d.data[0]).startAngle),
    )
    .append("title")
    .text((d) =>
      "group: ".concat(
  ...
  svg
    ...
    .attr("x", (d) =>
      widthScale(d[1].startAngle + (d[1].endAngle - d[1].startAngle) / 2),
    )
    .attr("y", yScale(0) + 15)
    .text((d) => d[0]);
...
function make_volume_chart(o_data, color, width, height, scale = null) {
  ...
  keys.forEach(function (key) {
    ...
    var _cube = makeCube(next_x, 0, 0, next_x + x_length, y_length, z_length);
  ...
  if (agents.length > 1) {
    ...
    // Don't draw the axis if these agents are not included
    yLine.push([0, 0, 0]);
    yLine.push([0, 0, 0]);
  }
  ...
  function processData(cubeData, yScaleData, xScaleData, zScaleData, tt) {
    ...
    var cubes = cubesGroup.selectAll("g.cube").data(cubeData, (d) => d.id);
    ...
    var texts = cubes
      ...
      .data(function (d) {
        ...
        return [
          { height: d.height, scenario: d.scenario, centroid: _t[0].centroid },
        ];
      });
    ...
    var yScale = svg.selectAll("path.yScale").data(yScaleData);
    ...
    var yText = svg.selectAll("text.yText").data(yScaleData);
    ...
    var xScale = svg.selectAll("path.xScale").data(xScaleData);
    ...
    var xText = svg.selectAll("text.xText").data(xScaleData);
    ...
    var zScale = svg.selectAll("path.zScale").data(zScaleData);
    ...
    var zText = svg.selectAll("text.zText").data(zScaleData);
  ...
  function dragged(event, d) {
    ...
    var new_cubes = cubes3D
      .rotateY(beta + startAngle)
      .rotateX(alpha - startAngle)(cubesData);
    var new_yScale = yScale3d
      .rotateY(beta + startAngle)
      .rotateX(alpha - startAngle)([yLine]);
    var new_xScale = xScale3d
      .rotateY(beta + startAngle)
      .rotateX(alpha - startAngle)([xLine]);
    var new_zScale = zScale3d
      .rotateY(beta + startAngle)
      .rotateX(alpha - startAngle)([zLine]);
