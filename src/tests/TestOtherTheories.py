...
class TestRunMFT(unittest.TestCase):
    ...
    def test_single_player(self):
        ...
        one_player_two_actions_split = {"1": {"A": 1}, "2": {"A": -1}}

        gameState = pm.ProportionalChancesGameState(
            {"A": 1}, one_player_two_actions_split
        )
        result = pm.run_mft(gameState)

        self.assertEqual(result, "1")

        one_player_two_actions_tie = {"1": {"A": 0}, "2": {"A": 0}}

    def test_zero_sum_two_player(self):
        outcomes = {"1": {"A": 1, "B": -1}, "2": {"A": -1, "B": 1}}
        ...
        gameState = pm.ProportionalChancesGameState({"A": 0.6, "B": 0.4}, outcomes)
        result = pm.run_mft(gameState)
        self.assertEqual(result, "1")

        gameState = pm.ProportionalChancesGameState({"A": 0.4, "B": 0.6}, outcomes)
        result = pm.run_mft(gameState)
        self.assertEqual(result, "2")
...
class TestRunMEC(unittest.TestCase):
    ...
    def test_single_player(self):
        ...
        # run_expectimax
        gameState = pm.ProportionalChancesGameState(
            {"A": 1}, one_player_two_actions_tie
        )
        result = pm.run_mec(gameState)
    ...
    def test_dominant_low_credence(self):
        action_agent_outcomes = {"1": {"A": 1, "C": -10000}, "2": {"A": -1, "C": 10000}}

        gameState = pm.ProportionalChancesGameState(
            {"A": 0.99, "C": 0.01}, action_agent_outcomes
        )
        result = pm.run_mec(gameState)
        self.assertEqual(result, "2")
...
class TestRandomDictatorPolicy(TestAssertArrays):
    ...
    def test_zero_sum_two_player(self):
        ...
        gameState = pm.ProportionalChancesGameState({"A": 0.5, "B": 0.5}, outcomes)
        result = pm.randomDictatorPolicy(gameState)
...
class TestRunNashBargain(TestAssertArrays):
    ...
    def test_single_player(self):
        ...
        gameState = pm.VoteGameState({"A": 0.001}, one_player_one_action)
        result = pm.run_nash_bargain(gameState)
    ...
    def test_zero_sum_two_player(self):
        ...
        gameState = pm.VoteGameState({"A": 0.6, "B": 0.4}, outcomes)
        result = pm.run_nash_bargain(gameState)
        self.assertEqual(result, "1")

        gameState = pm.VoteGameState({"A": 0.4, "B": 0.6}, outcomes)
        result = pm.run_nash_bargain(gameState)
        self.assertEqual(result, "2")
...
class TestLeximin(TestAssertArrays):
    ...
    def test_zero_pareto_two_player(self):
        outcomes = {"1": {"A": 1, "B": 1}, "2": {"A": 2, "B": 2}}

        gameState = pm.VoteGameState({"A": 1, "B": 1}, outcomes)
        result = pm.run_leximin(gameState)
        self.assertEqual(result, "2")

        outcomes = {"1": {"A": 1, "B": 1}, "2": {"A": 0, "B": 5}, "3": {"A": 2, "B": 2}}

        gameState = pm.VoteGameState({"A": 1, "B": 1}, outcomes)
        result = pm.run_leximin(gameState)
        self.assertEqual(result, "3")

        outcomes = {"1": {"A": 1, "B": 1}, "2": {"A": 1, "B": 5}, "3": {"A": 2, "B": 2}}

        gameState = pm.VoteGameState({"A": 1, "B": 1}, outcomes)
        result = pm.run_leximin(gameState)
        self.assertEqual(result, "3")

        outcomes = {"1": {"A": 1, "B": 1}, "2": {"A": 2, "B": 5}, "3": {"A": 3, "B": 3}}

        gameState = pm.VoteGameState({"A": 1, "B": 1}, outcomes)
        result = pm.run_leximin(gameState)
        self.assertEqual(result, "3")

        outcomes = {"1": {"A": 51, "B": 1, "C": 51}, "2": {"A": 1, "B": 1, "C": 51}}

        gameState = pm.VoteGameState({"A": 1, "B": 1}, outcomes)
        result = pm.run_leximin(gameState)
        self.assertEqual(result, "1")

        outcomes = {"1": {"A": 1, "B": 1, "C": 1}, "2": {"A": 51, "B": 51, "C": 51}}
...
class TestFairDominated(TestAssertArrays):
    ...
    def test_zero_pareto_two_player(self):
        ...
        outcomes = {"1": {"A": 1, "B": 1}, "2": {"A": 0, "B": 5}, "3": {"A": 2, "B": 1}}
...
class TestGiniIndex(TestAssertArrays):
    ...
    def test_multi_player(self):
        ...
        outcomes = {"1": {"A": 101, "B": 1, "C": 101}, "2": {"A": 1, "B": 101, "C": 1}}
