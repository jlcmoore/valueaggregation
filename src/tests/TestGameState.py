...
class TestProportionalChancesGameState(unittest.TestCase):
    ...
    def test_getNormalizedCredence(self):
        ...
        gs = pm.ProportionalChancesGameState({"A": 0.5, "B": 0.5}, {})
        self.assertEqual(gs.getNormalizedCredence("B"), 1 / 2)
    ...
    def test_varianceNormalizeOutcomes(self):
        ...
        expected_outcomes = {"1": {"A": 1.0, "B": -1.0}, "2": {"A": -1.0, "B": 1.0}}
        self.assertEqual(gameState.vote_agent_outcomes, expected_outcomes)

        outcomes = {"1": {"A": 1, "B": 0}, "2": {"A": 0, "B": 1}}
    ...
    def test_agentRangeNormalizeOutcomes(self):
        ...
        expected_outcomes = {"1": {"A": 3, "B": 1}, "2": {"A": 1, "B": 3}}
        self.assertEqual(gameState.vote_agent_outcomes, expected_outcomes)
        ...
        expected_outcomes = {"1": {"A": 2, "B": 1}, "2": {"A": 1, "B": 2}}
        self.assertEqual(gameState.vote_agent_outcomes, expected_outcomes)

        # should work with ties
        outcomes = {"1": {"A": 1, "B": 2}, "2": {"A": 1, "B": 2}}

        gameState = pm.ProportionalChancesGameState({"A": 1 / 2, "B": 1 / 2}, outcomes)
        gameState = pm.agentRangeNormalizeOutcomes(gameState)

        expected_outcomes = {"1": {"A": 1, "B": 1}, "2": {"A": 1, "B": 1}}
        self.assertEqual(gameState.vote_agent_outcomes, expected_outcomes)

        outcomes = {"1": {"A": -10, "B": 1}, "2": {"A": 10, "B": 21}}

        gameState = pm.ProportionalChancesGameState({"A": 1 / 2, "B": 1 / 2}, outcomes)
        gameState = pm.agentRangeNormalizeOutcomes(gameState)

        expected_outcomes = {"1": {"A": 1, "B": 1}, "2": {"A": 21, "B": 21}}
        self.assertEqual(gameState.vote_agent_outcomes, expected_outcomes)

        outcomes = {"1": {"A": -100, "B": 1}, "2": {"A": 10, "B": 21}}

        gameState = pm.ProportionalChancesGameState({"A": 1 / 2, "B": 1 / 2}, outcomes)
        gameState = pm.agentRangeNormalizeOutcomes(gameState)

        expected_outcomes = {"1": {"A": 1, "B": 1}, "2": {"A": 111, "B": 21}}
        self.assertEqual(gameState.vote_agent_outcomes, expected_outcomes)

        outcomes = {"1": {"A": +60, "B": +60}, "2": {"A": +10, "B": +100}}

        gameState = pm.ProportionalChancesGameState({"A": 1 / 2, "B": 1 / 2}, outcomes)
        gameState = pm.agentRangeNormalizeOutcomes(gameState)

        expected_outcomes = {"1": {"A": 51, "B": 1}, "2": {"A": 1, "B": 41}}
    ...
    def test_toArray(self):
        ...
        result = gameState.toArray()
        expected = [
            {"utility": 1, "action": "1", "agent": "A", "credence": 10},
...
class TestNegotiationGameState(unittest.TestCase):
    ...
    def test_generateCoalitions(self):
        ...
        self.assertEqual(
            actions,
            pm.NegotiationAction.fromList([({0}, "A"), ({1}, "A"), ({0, 1}, "A")]),
        )

        actions = pm.NegotiationGameState.generateCoalitions(["A", "B"], range(2))
        self.assertEqual(
            actions,
            pm.NegotiationAction.fromList(
                (
                    [
                        ({0}, "A"),
                        ({0}, "B"),
    ...
    def test_pruneCoalitions(self):
        ...
        pruned = pm.NegotiationGameState.pruneCoalitions(actions, 1, [])
        self.assertEqual(
            pruned, pm.NegotiationAction.fromList([({1}, "A"), ({0, 1}, "A")])
        )

        actions = pm.NegotiationGameState.generateCoalitions(["A"], range(2))
        pruned = pm.NegotiationGameState.pruneCoalitions(
            actions, 1, pm.NegotiationAction.fromList([({0, 1}, "A")])
        )
