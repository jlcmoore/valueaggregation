...
class ProportionalChancesGameState:
    ...
    def __init__(self, agents_to_credences, vote_agent_outcomes):
        ...
        self.actions_taken = (
            []
        )  # Assuming that these correspond to: list(agents_to_credences.keys())
    ...
    def getAgentUtilities(self, choice):
        ...
        return np.array(
            [self.vote_agent_outcomes[choice][agent] for agent in self.getAgents()]
        )
    ...
    def __repr__(self):
        return (
            f'{{"beliefs" : {repr(self.agents_to_credences)}, '
    ...
    def payoffFromVotes(gameState, votes):
        ...
        for (
            ...
            gameState.getVotes()
        ):  # have to calculate the expectation as this is a chance node
            nextUtilityVector = gameState.getAgentUtilities(vote)
            if (
                vote_probabilities[vote] != 0
            ):  # to deal with NaN from multiplying inf by 0
                for i in range(len(expectedValueVector)):
                    # These result is NaN otherwise
                    opposite_infinities = (
                        nextUtilityVector[i] == float("inf")
                        and expectedValueVector[i] == float("-inf")
                    ) or (
                        nextUtilityVector[i] == float("-inf")
                        and expectedValueVector[i] == float("inf")
                    )
                    if not opposite_infinities:
                        expectedValueVector[i] += (
                            nextUtilityVector[i] * vote_probabilities[vote]
                        )
    ...
    def toArray(self):
        ...
        for action in self.vote_agent_outcomes:
            for agent in self.vote_agent_outcomes[action]:
                output.append(
                    {
                        "utility": self.vote_agent_outcomes[action][agent],
                        "agent": agent,
    ...
    def __eq__(self, other):
        if isinstance(other, type(self)):
            return (
                self.vote_agent_outcomes == other.vote_agent_outcomes
                and self.agents_to_credences == other.agents_to_credences
...
def normalizeOutcomes(gameState, norm_range):
    ...
    for vote in newGameState.vote_agent_outcomes:
        for agent in newGameState.vote_agent_outcomes[vote]:
            ...
            newGameState.vote_agent_outcomes[vote][agent] = np.interp(
                outcome, agent_ranges[agent], norm_range
            )
    return newGameState
...
def bordaNormalizeOutcomes(gameState):
    ...
    for vote in newGameState.vote_agent_outcomes:
        for agent in newGameState.vote_agent_outcomes[vote]:
            ...
            agent_queues[agent].put(
                (newGameState.vote_agent_outcomes[vote][agent], vote)
            )
...
def generate_n_by_m_game(
    num_players, num_actions, equal_rewards=True, gameType=ProportionalChancesGameState
):
    """
...
class NegotiationGameState(ProportionalChancesGameState):
    ...
    def __init__(self, agents_to_credences, vote_agent_outcomes):
        ...
        self.possible_actions = type(self).generateCoalitions(
            self.getVotes(), self.getAgents()
        )
    ...
    def getLegalActions(self, agentIndex):
        ...
        if agentIndex < self.numAgents():  # branch node
            ...
            actions = type(self).pruneCoalitions(
                self.possible_actions, agent, self.actions_taken
            )
        else:  # chance node
    ...
    def generateCoalitions(cls, votes, agents):
        coalition_lists = [
            itertools.combinations(agents, i) for i in range(1, len(agents) + 1, 1)
        ]
        coalitions = itertools.chain(*coalition_lists)
        vote_coalition_combos = list(itertools.product(coalitions, votes))
        actions = [
            cls.getActionType()(*coalition) for coalition in vote_coalition_combos
        ]
        return actions
...
def generateCoalitions(cls, votes, agents):
    ...
    combo_lists = [
        itertools.combinations(votes, i) for i in range(1, len(votes) + 1, 1)
    ]
...
class VoteGameState(ProportionalChancesGameState):
    ...
    def __init__(
        ...
        agents_to_credences,
        vote_agent_outcomes,
        defaultPolicyFunction=None,
        defaultPolicyVector=False,
    ):
        """
        Expects a vector of all utilities if defaultPolicyVector is `True`
        """
        super().__init__(agents_to_credences, vote_agent_outcomes)
        self.majority_point = 0.5
