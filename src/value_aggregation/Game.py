...
def expectiMax(gameState, depth, agentIndex, chanceNode=True):
    ...
    for action in gameState.getLegalActions(agentIndex):
        ...
        if chanceNode and nextAgentIndex >= gameState.numAgents():
            nextUtilityVector, nextVotes = expectiChance(
                nextState, depth, nextAgentIndex
            )
        else:
            tmp_depth = depth
            if not chanceNode and nextAgentIndex >= gameState.numAgents():
                tmp_depth -= 1
            nextUtilityVector, nextVotes = expectiMax(
                nextState, tmp_depth, nextAgentIndex, chanceNode
            )

        if (
            bestUtilityVector is None
            or nextUtilityVector[agentIndex] > bestUtilityVector[agentIndex]
        ):
    ...
    logger.debug(
        f"{tabs}{gameState.getAgent(agentIndex)}: {bestUtilityVector}, {bestVotes}"
    )
...
def run_expectimax(gameState):
    ...
    logger.debug(
        f"Players: {gameState.getAgents()}, actions: {actions}, utilities: {utilities}"
    )

    if max_action_percent <= 0.5:
        return None
    return max_action
...
def to_agent_payoffs(gameState):
    ...
    payoffs_per_agent_arrays = list(
        map(lambda x: np.array(x, dtype=gb_data_type).reshape(shape), payoffs_per_agent)
    )
...
def run_mec(gameState, none_if_tie=False):
    ...
    for vote in gameState.vote_agent_outcomes:
        ...
        for agent in gameState.vote_agent_outcomes[vote]:
            votes_to_utilities[vote] += gameState.vote_agent_outcomes[vote][
                agent
...
def run_fair_dominated(gameState, none_if_tie=True):
    ...
    modes_and_counts_sorted = sorted(modes_and_counts, key=lambda x: x[1], reverse=True)
    max_count = modes_and_counts_sorted[0][1]
    max_count_only = list(filter(lambda x: x[1] == max_count, modes_and_counts_sorted))

    max_count_only_mode_sorted = sorted(
        max_count_only, key=lambda x: x[0], reverse=True
    )

    # This means there is a tie for the min element
    if (
        len(max_count_only_mode_sorted) > 1
        and max_count_only_mode_sorted[1][0] >= max_count_only_mode_sorted[0][0]
    ):
        return None
...
def run_equality_efficiency(gameState, none_if_tie=True, alpha=0.5, asymmetric=False):
    ...
    for vote in gameState.vote_agent_outcomes:
        ...
        efficiency = 0
        for agent in gameState.vote_agent_outcomes[vote]:
            efficiency += gameState.vote_agent_outcomes[vote][
                agent
            ] * gameState.getNormalizedCredence(agent)
