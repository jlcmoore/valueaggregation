...
from .Game import run_equality_efficiency, run_mec, run_mft, run_nash_bargain
...
def make_belief_range(belief_range, belief_range_steps, num_agents, belief_norm):
    ...
    denormalize_partial = functools.partial(
        denormalize, num_agents=num_agents, belief_norm=belief_norm
    )
...
def generate_games(
    agents,
    actions,
    belief_range_steps=2,
    action_range_steps=2,
    action_range=(1, 101),
    belief_range=(0.01, 0.99),
    belief_norm=100,
    ...
    beliefs = make_belief_range(
        belief_range, belief_range_steps, num_agents, belief_norm
    )
    ...
    if max_actions is not None:
        ...
        agents_by_actions = random.sample(
            agents_by_actions, min(max_actions, len(agents_by_actions))
        )
    # outcomes = itertools.product(agents_by_actions, repeat=num_actions)
    outcomes = itertools.combinations(agents_by_actions, r=num_actions)
    ...
    game_states = [
        gameState(*add_dict_keys_to_lists(*game, agents, actions)) for game in games
    ]
    return game_states
...
def _make_mixed_hit_list(
    disagreements,
    non_disagreements,
    scenarios_per_hit,
    disagreements_per_hit,
):
    """Build shuffled HIT rows with a fixed disagreement/agreement composition."""
    if scenarios_per_hit <= 0:
        raise ValueError("scenarios_per_hit must be positive.")
    if disagreements_per_hit < 0 or disagreements_per_hit > scenarios_per_hit:
        raise ValueError("disagreements_per_hit must be in [0, scenarios_per_hit].")

    non_disagreements_per_hit = scenarios_per_hit - disagreements_per_hit
    if disagreements_per_hit > 0 and len(disagreements) == 0:
        raise ValueError(
            "Requested disagreements_per_hit > 0 but no disagreement scenarios exist."
        )
    if non_disagreements_per_hit > 0 and len(non_disagreements) == 0:
        raise ValueError(
            "Requested agreements per hit > 0 but no agreement scenarios exist."
        )

    disagreement_rows = (
        split_by(disagreements, disagreements_per_hit)
        if disagreements_per_hit > 0
        else [[]]
    )
    non_disagreement_rows = (
        split_by(non_disagreements, non_disagreements_per_hit)
        if non_disagreements_per_hit > 0
        else [[]]
    )

    row_count = max(len(disagreement_rows), len(non_disagreement_rows))
    hit_list = []
    for row_index in range(row_count):
        row = []
        row += list(disagreement_rows[row_index % len(disagreement_rows)])
        row += list(non_disagreement_rows[row_index % len(non_disagreement_rows)])
        random.shuffle(row)
        hit_list.append(row)
...
def generate_hits_greedy(
    ...
    disagreements_per_hit=None,
):

    agent_interval = list(
        map(math.floor, np.linspace(*belief_range, belief_range_steps) * belief_norm)
    )
    action_interval = action_range_func(*action_range, action_range_steps)

    if disagrees_only:
        disagreements_target = sample_size
        non_disagreements_target = 0
    else:
        disagreements_target = math.floor(sample_size / 2)
        non_disagreements_target = sample_size - disagreements_target

    games_generated = 0
    disagreements = set()
    non_disagreements = set()
    while (
        len(disagreements) < disagreements_target
        or len(non_disagreements) < non_disagreements_target
    ):
        game = generate_game(
            agent_interval, agents, action_interval, actions, gameState
        )
        games_generated += 1
        ...
        if disagreement and len(disagreements) < disagreements_target:
            disagreements.add(game)
        elif not disagreement and len(non_disagreements) < non_disagreements_target:
            non_disagreements.add(game)

    non_disagreements_labeled = list(map(encode_hit, non_disagreements))
    disagreements_labeled = list(map(encode_hit, disagreements))
    ...
    if disagreements_per_hit is None:
        disagreements_per_hit = math.floor(scenarios_per_hit / 2)

    return _make_mixed_hit_list(
        disagreements=disagreements_labeled,
        non_disagreements=non_disagreements_labeled,
        scenarios_per_hit=scenarios_per_hit,
        disagreements_per_hit=disagreements_per_hit,
    )
...
def is_not_tie(gameState, aggregation_functions):
    ...
    return counts[None] == 0 and (
        len(counts) == 1 or len(counts) == len(aggregation_functions)
    )
...
def encode_hit(gameState):
    ...
    result = {
        ...
        "fehr": run_equality_efficiency(gameState, none_if_tie=True, alpha=0.3),
    }
...
def generate_hits(
    ...
    is_not_tie_partial = functools.partial(
        is_not_tie, aggregation_functions=aggregation_functions
    )
    disagrees_func_partial = functools.partial(
        disagrees_func, aggregation_functions=aggregation_functions
    )
    ...
    if disagrees_only:
        disagreements_target = min(sample_size, len(disagreements))
        non_disagreements_target = 0
    else:
        disagreements_target = min(math.floor(sample_size / 2), len(disagreements))
        non_disagreements_target = min(
            sample_size - disagreements_target, len(no_disagreements)
        )

    disagreements_sample = random.sample(list(disagreements), disagreements_target)

    # Sample the same number from remaining
    no_disagreements_sample = random.sample(
        list(no_disagreements), non_disagreements_target
    )

    no_disagreements_labeled = list(map(encode_hit, no_disagreements_sample))
    disagreements_labeled = list(map(encode_hit, disagreements_sample))
    ...
    return _make_mixed_hit_list(
        disagreements=disagreements_labeled,
        non_disagreements=no_disagreements_labeled,
        scenarios_per_hit=scenarios_per_hit,
        disagreements_per_hit=disagreements_per_hit,
    )
